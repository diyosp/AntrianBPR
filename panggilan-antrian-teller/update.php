    ...
    if (mysqli_num_rows($check_query) > 0) {
      ...
      if ($action === 'start') {
        ...
        if ($status_teller == '1') {
          ...
          $update = mysqli_query($mysqli, "UPDATE tbl_antrian_teller SET status_teller='2', updated_date_teller='$updated_date', waktu_selesai='$waktu_selesai', durasi='$durasi', jumlah_transaksi='$jumlah_transaksi' WHERE id_teller='$id' AND cabang_id='$cabang_id'")
            or die('Ada kesalahan pada query update : ' . mysqli_error($mysqli));
        }
      }
